...
colnames(Y) <- "activity"
Y$activitylabel <- factor(Y$activity, labels = as.character(label[,2]))
activitylabel <- Y[,-1]
...
tidy <- cbind(X, activitylabel, Sub)
tidy_mean <- tidy %>% group_by(activitylabel, subject) %>% summarize_all(funs(mean))
